...
const PaymentDebt175Page = () => {
  const { t } = useTranslation();
  const [total] = useState(175);
  const [loadingPaymentInfo, setLoadingPaymentInfo] = useState(true);
  const [transactionIds, setTransactionIds] = useState<
    Array<{ id: string; type: string; amount: number; to: string }>
  >([]);
  ...
  const onGetPaymentInfo = async () => {
    ...
    await Payment.getDebt175PaymentInfo()
      .then((response: any) => {
  ...
  const donePayment = useCallback(
    async (transactionHash: string) => {
      await Payment.onDoneDebt175Payment({
        transIds: transactionIds,
        transactionHash,
      })
        .then((response: any) => {
            ...
            {!paymentCompleted && showPayment && (
              <>
                <div className="w-full max-w-203 mx-auto rounded-lg bg-white p-10 text-gray-700 mt-4">
                  ...
                  <div className="flex justify-between mb-6">
                    <div className="mb-3">
                      <p className="text-lg mb-2 ml-1">
                        <span className="font-bold">Loại thanh toán</span> : 175
                        USDT
                      </p>
                    </div>
                    <div className="mb-3">
                  ...
                  {transactionIds.map((payment) => {
                    let bgColor = 'bg-blue-50 text-blue-800';
                    if (payment.type.includes('COMPANY')) {
                      bgColor = 'bg-purple-100 text-purple-800';
                    } else if (payment.type.includes('PIG')) {
                      bgColor = 'bg-pink-100 text-pink-800';
                    } else if (payment.type.includes('DIRECT')) {
                    ...
                    return (
                      <div
                        key={payment.id}
                        className={`flex items-center p-4 mb-4 text-sm rounded-lg ${bgColor}`}
                        role="alert"
                      >
                        <svg
                        ...
                        <div className="w-full flex flex-col sm:flex-row justify-between gap-2">
                          <div className="">
                            <span className="font-medium">
                              {payment.type.includes('COMPANY')
                                ? 'HEWE'
                                : payment.type.includes('PIG')
                                ? 'DreamPool'
                                : payment.type.includes('DIRECT')
                                ? t('commissionFee')
                                : ''}
                              <span> : </span>
                            </span>
            ...
            {paymentCompleted && (
              <div>
                <div className="flex flex-col items-center lg:gap-10 gap-4">
                  ...
                  <p className="text-md lg:text-2xl font-bold">Cảm ơn bạn!</p>
                </div>
              </div>
            )}
          </>
...
export default PaymentDebt175Page;
