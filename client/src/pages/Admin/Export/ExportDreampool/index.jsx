...
const ExportDreampoolPage = () => {
  ...
  const exportDreampool = async (values) => {
    const limit = 100;
    ...
    const fetchData = async () => {
      try {
        ...
        exportToExcel(
          excelData,
          `Danh_sach_dreampool_${moment().format('DD/MM/YYYY_HH:mm:ss')}`,
        );
        setLoading(false);
  ...
  const getData = async (body) => {
    try {
      return await User.exportDreamPool(body).then((response) => {
        ...
        <div className="mx-auto sm:container mb-10">
          <div>
            <h2 className="text-dark mb-2 text-2xl font-semibold dark:text-white">
              Export Dreampool List
            </h2>
            <p className="text-body-color dark:text-dark-6 text-sm font-medium">
              Download and manage the list of dreampool in Excel format for
              reporting and administrative purposes.
            </p>
          </div>
        </div>
        <form onSubmit={handleSubmit(exportDreampool)} className="">
...
export default ExportDreampoolPage;
