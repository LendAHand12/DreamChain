...
import moveSystemStatus from '@/constants/moveSystemStatus';
...
import MoveSystem from '@/api/MoveSystem';
import { shortenWalletAddress } from '@/utils';
import { useSelector } from 'react-redux';
import CustomPagination from '@/components/CustomPagination';

const MoveSysteListPage = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      await MoveSystem.list(objectFilter)
        .then((response) => {
  ...
  const pushParamsToUrl = (pageNumber, searchStatus, keyword) => {
    ...
    const url = queryString
      ? `/admin/move-system-list?${queryString}`
      : '/admin/move-system-list';
    navigate(url);
  };
...
export default MoveSysteListPage;
